...
void BlurFilter::Execute(ID3D12GraphicsCommandList* cmdList,
	...
	int blurCount)
{
	static const float threadsPerGroup = 256.0f;
	...
	for (int i = 0; i < blurCount; ++i)
	{
		...
		UINT numGroupsX = (UINT)ceilf(mWidth / threadsPerGroup);
		...
		// group covers 256 pixels  (the 256 is defined in the ComputeShader).
		UINT numGroupsY = (UINT)ceilf(mHeight / threadsPerGroup);
...
void BlurFilter::ExecuteBilateralBlur(ID3D12GraphicsCommandList* cmdList,
	ID3D12RootSignature* rootSig, ID3D12PipelineState* horzBlurPSO,
	ID3D12PipelineState* vertBlurPSO, ID3D12Resource* input, int blurCount)
{
	static const float threadsPerGroup = 256.0f;
	static const float sigmaR = 0.1f;
	
	// 
	auto weights = CalcBilateralSpatialWeightsUnorm(6.0f);
	int blurRadius = weights.size() / 2;
	float twoSigmaRSquared = 2.0f*sigmaR*sigmaR;


	cmdList->SetComputeRootSignature(rootSig);
	cmdList->SetComputeRoot32BitConstants(0, 1, &blurRadius, 0);
	cmdList->SetComputeRoot32BitConstants(0, 1, &twoSigmaRSquared, 1);
	...
	for (int i = 0; i < blurCount; ++i) {
		...
		// prepare for vert blur
		cmdList->ResourceBarrier(1, &CD3DX12_RESOURCE_BARRIER::Transition(
			mBlurMap0.Get(), D3D12_RESOURCE_STATE_GENERIC_READ, D3D12_RESOURCE_STATE_UNORDERED_ACCESS));

		cmdList->ResourceBarrier(1, &CD3DX12_RESOURCE_BARRIER::Transition(
			mBlurMap1.Get(), D3D12_RESOURCE_STATE_UNORDERED_ACCESS, D3D12_RESOURCE_STATE_GENERIC_READ));
		...
		cmdList->ResourceBarrier(1, &CD3DX12_RESOURCE_BARRIER::Transition(
			mBlurMap0.Get(), D3D12_RESOURCE_STATE_UNORDERED_ACCESS, D3D12_RESOURCE_STATE_GENERIC_READ));

		cmdList->ResourceBarrier(1, &CD3DX12_RESOURCE_BARRIER::Transition(
			mBlurMap1.Get(), D3D12_RESOURCE_STATE_GENERIC_READ, D3D12_RESOURCE_STATE_UNORDERED_ACCESS));
	}

	// restore original states of resource

	cmdList->ResourceBarrier(1, &CD3DX12_RESOURCE_BARRIER::Transition(
		mBlurMap0.Get(), D3D12_RESOURCE_STATE_GENERIC_READ, D3D12_RESOURCE_STATE_COMMON));

	cmdList->ResourceBarrier(1, &CD3DX12_RESOURCE_BARRIER::Transition(
		mBlurMap1.Get(), D3D12_RESOURCE_STATE_UNORDERED_ACCESS, D3D12_RESOURCE_STATE_COMMON));
}
...
std::vector<float> BlurFilter::CalcBilateralSpatialWeightsUnorm(float sigma) {
	const float twoSigmaSquared = 2.0f*sigma*sigma;

	int blurRadius = min(ceil(2.0f * sigma), 1);
	...
	for (int i = -blurRadius; i <= blurRadius; ++i) {
		auto x = (float)i;
		weights[i + blurRadius] = expf(-x*x / twoSigmaSquared);
	}
